...
from .settings import StrategyProfile, settings
...
def profile_blackout_bounds(profile: StrategyProfile) -> tuple[time | None, time | None]:
    return parse_time_of_day(profile.blackout_start), parse_time_of_day(profile.blackout_end)


def in_profile_blackout(ts: pd.Timestamp | str | None, profile: StrategyProfile) -> bool:
    if ts is None:
        return False
    blackout_start, blackout_end = profile_blackout_bounds(profile)
    if blackout_start is None or blackout_end is None:
        return False
    return pd.Timestamp(ts).time() >= blackout_start and pd.Timestamp(ts).time() < blackout_end


def parse_face_policy_id(policy_id: str) -> tuple[tuple[int, str], tuple[int, str], tuple[int, str], int]:
    parts = str(policy_id).split("__")
    if len(parts) != 4:
        raise ValueError(f"Unsupported face policy id: {policy_id}")

    def parse_gate(part: str, prefix: str) -> tuple[int, str]:
        if not part.startswith(prefix):
            raise ValueError(f"Unsupported face policy id: {policy_id}")
        payload = part[len(prefix):]
        window_text, family = payload.split("_", 1)
        return int(window_text), family

    cooldown_part = parts[3]
    if not cooldown_part.startswith("cd"):
        raise ValueError(f"Unsupported face policy id: {policy_id}")
    return (
        parse_gate(parts[0], "core"),
        parse_gate(parts[1], "exp"),
        parse_gate(parts[2], "oe"),
        int(cooldown_part[2:]),
    )
...
def build_extended_face_bundle(mods: StrategyModules, issue_df: pd.DataFrame, profile: StrategyProfile):
    blackout_start, blackout_end = profile_blackout_bounds(profile)
    work = filter_blackout(issue_df, blackout_start, blackout_end)
...
def build_extended_odd_even_bundle(mods: StrategyModules, issue_df: pd.DataFrame, profile: StrategyProfile):
    blackout_start, blackout_end = profile_blackout_bounds(profile)
    work = filter_blackout(issue_df, blackout_start, blackout_end)
    pos_cols = [f"pos{i}" for i in range(1, 11)]
...
def build_face_context(
    mods: StrategyModules,
    issue_df: pd.DataFrame,
    current_date: pd.Timestamp,
    profile: StrategyProfile,
) -> dict[str, Any]:
    bundle, _slot_mask, schedule_df = build_extended_face_bundle(mods, issue_df, profile)
    round9 = mods.round9
    ...
    oe_bundle, _oe_mask, _oe_schedule = build_extended_odd_even_bundle(mods, issue_df, profile)
    ...
    core_cfg, exp_cfg, oe_cfg, cooldown_days = parse_face_policy_id(profile.face_policy_id)
    trace = round35.simulate_policy(
        df=df,
        policy_id=profile.face_policy_id,
        core_cfg=core_cfg,
        exp_cfg=exp_cfg,
        oe_cfg=oe_cfg,
...
def build_sum_context(mods: StrategyModules, issue_df: pd.DataFrame, profile: StrategyProfile) -> dict[str, Any]:
    bundle, _slot_mask, schedule_df = build_extended_sum_bundle(mods, issue_df)
    candidate_row = load_sum_candidate_row(mods, profile.sum_candidate_id)
...
def build_exact_context(mods: StrategyModules, issue_df: pd.DataFrame, profile: StrategyProfile) -> dict[str, Any]:
    ...
    filtered = rule_state_df[
        (rule_state_df["base_gate_id"] == profile.exact_base_gate_id)
        & (rule_state_df["obs_window"] == profile.exact_obs_window)
    ].copy()
    rule_col = f"rule_{profile.exact_execution_rule}"
    filtered["execute_exact"] = filtered[rule_col].astype(bool)
    filtered["selected_number_exec"] = filtered.apply(
        lambda row: mods.number_window.selected_number_for_rule(profile.exact_execution_rule, row),
        axis=1,
    )
...
def current_day_issue_maps(
    ...
    blackout_start, blackout_end = profile_blackout_bounds(profile)
    face_day = filter_blackout(raw_day, blackout_start, blackout_end).copy()
    face_day["slot_1based"] = np.arange(1, len(face_day) + 1)
    return raw_day, face_day


def build_live_sum_plan(
    mods: StrategyModules,
    sum_ctx: dict[str, Any],
    ...
    allowed_slots = set(
        raw_day.loc[~raw_day["draw_ts"].map(lambda ts: in_profile_blackout(ts, profile)), "slot_1based"].astype(int).tolist()
    )
    ...
    tradable_slots = [
        ...
        if slot >= preview_cut and slot < len(score) and int(slot + 1) in allowed_slots
    ]
    metrics = {
...
def build_live_exact_plan(
    ...
    latest_slot = int(len(raw_day))
    if latest_slot < profile.exact_obs_window:
        return {
            "status": "waiting_preview",
            "latest_slot": latest_slot,
            "requested_slots": 0,
            "picks": [],
            "message": f"等待前 {profile.exact_obs_window} 期完成判窗",
        }
    current_week_idx = int(len(exact_ctx["bundle"].round9_bundle.week_start) - 1)
    current_rows = exact_ctx["subgroup_state_df"][
        (exact_ctx["subgroup_state_df"]["base_gate_id"] == profile.exact_base_gate_id)
    ...
    for row in current_rows.itertuples(index=False):
        ...
        if slot_1based <= profile.exact_obs_window or slot_1based > exact_ctx["bundle"].round9_bundle.n_slots:
            continue
        if slot_1based not in allowed_slots:
            continue
        prefix_seq = number_matrix[: profile.exact_obs_window, int(row.position)].astype(int)
...
def finalize_live_state(
    ...
    sum_plan = build_live_sum_plan(mods, sum_ctx, raw_day, profile)
    exact_plan = build_live_exact_plan(mods, exact_ctx, raw_day, profile)
    ...
    next_trade_allowed = True
    draw_time = pd.Timestamp(live_payload["drawTime"]) if live_payload.get("drawTime") else None
    if draw_time is not None:
        next_trade_allowed = not in_profile_blackout(draw_time, profile)

    maybe_append("face", face_pending, next_face_slot if next_trade_allowed else None)
    maybe_append("sum", sum_pending, next_raw_slot if next_trade_allowed else None)
    maybe_append("exact", exact_pending, next_raw_slot if next_trade_allowed else None)
...
def build_historical_bet_rows(
    ...
    def current_live_issue_for_face(slot_1based: int) -> int | None:
        latest_issue = market.get("pre_draw_issue")
        latest_slot = market.get("face_latest_slot")
        if latest_issue is None or latest_slot is None:
            return None
        return int(latest_issue) - int(latest_slot) + int(slot_1based)
            ...
            for bucket_name in ("executed", "pending"):
                ...
                for item in line_state[bucket_name]:
                    ...
                    if row_issue is None:
                        if line_name == "face":
                            row_issue = current_live_issue_for_face(int(item["slot_1based"])) or row_issue
                        elif line_name in {"sum", "exact"}:
...
def build_runtime_context(mods: StrategyModules, issue_df: pd.DataFrame, profile: StrategyProfile) -> dict[str, Any]:
    if issue_df.empty:
    ...
    face_ctx = build_face_context(mods, issue_df, current_date, profile)
    sum_ctx = build_sum_context(mods, issue_df, profile)
    exact_ctx = build_exact_context(mods, issue_df, profile)
...
def snapshot_from_context(
    ...
    profile: StrategyProfile = context["profile"]
    ...
    raw_day, face_day = current_day_issue_maps(issue_df, current_date, profile)
    live_state = finalize_live_state(mods, current_date, raw_day, face_day, replay, face_ctx, sum_ctx, exact_ctx, live_payload, profile)
    market = {
...
def build_snapshot(mods: StrategyModules, issue_df: pd.DataFrame, live_payload: dict[str, Any]) -> dict[str, Any]:
    context = build_runtime_context(mods, issue_df, settings.primary_profile)

...
class LiveRuntime:
    def __init__(self) -> None:
        ...
        self._primary_profile = settings.primary_profile
        self.snapshot: dict[str, Any] = {
            "generated_at": None,
            "market": {},
            "active_profile_id": self._primary_profile.id,
            "history_scope": "primary_only",
            "profiles": {},
            "totals": {},
            "contributions": {},
            "line_state": {},
    ...
    def _curve_rows_from_snapshot(self, snapshot: dict[str, Any] | None) -> list[dict[str, Any]]:
        if not snapshot:
            return []
        ...
        if isinstance(profiles, dict):
            primary = profiles.get(self._primary_profile.id) or {}
            rows = primary.get("daily_curve")
            if isinstance(rows, list):
                return rows
        return []

    def _public_profile_snapshot(self, snapshot: dict[str, Any]) -> dict[str, Any]:
        return {k: v for k, v in snapshot.items() if k not in {"replay", "bet_rows"}}

    def _compose_dashboard_snapshot(self, profile_snapshots: dict[str, dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any]]:
        public_profiles = {
            profile.id: _jsonify(self._public_profile_snapshot(profile_snapshots[profile.id]))
            for profile in self._profiles
            if profile.id in profile_snapshots
        }
        primary_public = public_profiles[self._primary_profile.id]
        combined = {
            "generated_at": primary_public.get("generated_at"),
            "market": primary_public.get("market", {}),
            "active_profile_id": self._primary_profile.id,
            "history_scope": "primary_only",
            "profiles": public_profiles,
            "ranges": primary_public.get("ranges", {}),
            "totals": primary_public.get("totals", {}),
            "contributions": primary_public.get("contributions", {}),
            "line_state": primary_public.get("line_state", {}),
            "today_plan": primary_public.get("today_plan", {}),
            "current_actions": primary_public.get("current_actions", []),
            "daily_curve": primary_public.get("daily_curve", []),
        }
        return combined, primary_public

    async def startup(self) -> None:
        db.ensure_runtime_tables()
        persisted = db.read_runtime_state("dashboard")
        if persisted:
            curve = self._curve_rows_from_snapshot(persisted)
    ...
    def persist_daily_equity(self, daily_curve: list[dict[str, Any]]) -> None:
        db.execute("DELETE FROM pk10_daily_equity WHERE draw_date < %s", (settings.simulation_start_date,))
        rows = []
        ...
        db.executemany(
            ...
            ON DUPLICATE KEY UPDATE
                settled_bankroll = VALUES(settled_bankroll),
                total_real_pnl = VALUES(total_real_pnl),
                face_real_pnl = VALUES(face_real_pnl),
                sum_real_pnl = VALUES(sum_real_pnl),
                exact_real_pnl = VALUES(exact_real_pnl),
    ...
    def persist_bet_rows(self, bet_rows: list[dict[str, Any]], live_date: str) -> None:
        db.execute("DELETE FROM pk10_bet_log WHERE draw_date < %s", (settings.simulation_start_date,))
        db.execute("DELETE FROM pk10_bet_log WHERE status = %s AND draw_date < %s", ("pending", live_date))
        db.execute("DELETE FROM pk10_bet_log WHERE draw_date = %s AND status = %s", (live_date, "pending"))
        rows = [
            (
                item["draw_date"],
        ...
        db.executemany(
            ...
            ON DUPLICATE KEY UPDATE
                pre_draw_issue = VALUES(pre_draw_issue),
    ...
    def persist_broadcast_rows(self, broadcast_rows: list[dict[str, Any]]) -> None:
        db.execute("DELETE FROM pk10_broadcast_log WHERE draw_date < %s", (settings.simulation_start_date,))
        rows = [
            (
        ...
        db.executemany(
            ...
            ON DUPLICATE KEY UPDATE
                ...
                pre_draw_issue = VALUES(pre_draw_issue),
                latest_slot = VALUES(latest_slot),
    ...
    def _refresh_sync(self, force_issue_log: bool = False, live_payload: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        need_full_rebuild = self._issue_df is None or not self._contexts or self._context_date != live_date
        if need_full_rebuild:
            self.sync_missing_history(live_payload)
            self._issue_df = load_issue_history_from_db(settings.history_start_date)
            self._contexts = {
                profile.id: build_runtime_context(self.modules, self._issue_df, profile)
                for profile in self._profiles
            }
            self._context_date = live_date
        self.append_live_issue_to_cache(live_payload)
        profile_snapshots = {
            profile.id: snapshot_from_context(self.modules, self._issue_df, live_payload, self._contexts[profile.id])
            for profile in self._profiles
        }
        primary_snapshot = profile_snapshots[self._primary_profile.id]
        public_snapshot, primary_public_snapshot = self._compose_dashboard_snapshot(profile_snapshots)
        annotated_bet_rows, broadcast_rows = self.build_broadcast_views(_jsonify(primary_snapshot["bet_rows"]), primary_public_snapshot)
        self.persist_daily_equity(primary_public_snapshot["daily_curve"])
        self.persist_bet_rows(annotated_bet_rows, live_date)
        if force_issue_log:
